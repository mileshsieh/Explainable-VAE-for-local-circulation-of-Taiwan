...
def plotStreamLine(ax,topo,vardata,title,rtitle,ltitle):
    ...
    ax.set_title(rtitle,fontsize=15,loc='right')
    ax.set_title(ltitle,fontsize=15,loc='left')
    ax.set_title(title,fontsize=23,loc='center')
    return strm

def plotPatternPhase(xx,yy,pattern,cnt,title):
    ...
    for ii in range(nx_ldim):
        for jj in range(ny_ldim):
            strm=plotStreamLine(axes[jj,ii],topo,pattern[ny_ldim-jj-1,ii,:,:,:],title_ch[jj*nx_ldim+ii],'(%.1f,%.1f)'%(xx[ny_ldim-jj-1,ii],yy[ny_ldim-jj-1,ii]),'N=%d'%cnt[jj,ii])
...
if __name__=='__main__':
  ...
  latent_mu_all=np.load('data/VAE/latent/latent_var.%s.npy'%sf)
  ...
  xbins=ybins=np.linspace(-2,2,5)

  xx,yy=np.meshgrid(0.5*(xbins[:-1]+xbins[1:]),0.5*(ybins[:-1]+ybins[1:]))
  ny_ldim,nx_ldim=xx.shape

  #calculate density of smaple size in evenly splitted 2D latent space
  xy=latent_mu_all.reshape(nCase*nt,latent_dim)
  ...
  print('%d of %d (%.2f) samples are included in the sampling subspace'%(int(cnt.sum()),nCase*nt,100*cnt.sum()/(nCase*nt)))
